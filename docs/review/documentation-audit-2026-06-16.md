...
- Operations docs now agree that the Telegram bot is part of default production Compose, only one polling process may use the shared Telegram bot token, n8n should not publish port `5678` directly, API-to-n8n calls should use `http://n8n:5678` inside Docker, and public launch remains blocked until domains/HTTPS, direct test-port closure, Dokploy admin access restriction, disposable staff-route smoke, and real database transfer smoke checks are complete.
...
- Public launch is still not approved: domains and HTTPS are missing, temporary direct test ports must be closed, Dokploy admin access should be restricted to trusted IP/VPN access, disposable staff-route smoke must be rerun, and smoke/backup/restore checks must be repeated after real database transfer.
