...
This audit reviewed the documentation harness after Phase 16 inventory reservations, the post-phase production hardening commits, and the self-hosted n8n VPS handoff on June 16, 2026. The goal was to verify that the repository can guide the next implementation slice without relying on chat history, and that current docs do not send future workers toward issues already fixed in code.
...
- The self-hosted n8n VPS evidence existed as an operations artifact but was not yet part of the repository map, project history, or documentation harness checks. Those entry points now record the production handoff from n8n Cloud to private Compose-network n8n URLs, the verified request-created delivery, the one-active-Telegram-polling rule, and the current port posture.
...
- Architecture, product, domain, operations, and repository-map docs agree on the implemented runtime: FastAPI API, React/Vite web, Celery worker, Telegram bot, PostgreSQL/pgvector, Redis, self-hosted n8n on the production Compose network, deterministic local providers, OpenAI-compatible live adapters, safe structured logs, staff audit expansion, scheduling depth, inventory reservations, production concurrency guards, operational diagnostics, incident response, and VPS/Dokploy deployment evidence.
...
- Operations docs now agree that the Telegram bot is part of default production Compose, only one polling process may use the shared Telegram bot token, n8n should not publish port `5678` directly, API-to-n8n calls should use `http://n8n:5678` inside Docker, and public launch remains blocked until domains/HTTPS, direct test-port closure, Dokploy admin access restriction, disposable staff-route smoke, setup-secret rotation, and real database transfer smoke checks are complete.
...
- Public launch is still not approved: domains and HTTPS are missing, temporary direct test ports must be closed, Dokploy admin access should be restricted to trusted IP/VPN access, disposable staff-route smoke must be rerun, setup-exposed secrets must be rotated, and smoke/backup/restore checks must be repeated after real database transfer.
...
Current documentation quality after this audit: **9.5/10**.

The documentation is strong enough for backlog grooming and next-slice planning. The strongest areas are entry-point clarity, phase sequencing, domain boundaries, public/private safety rules, operations runbooks, production hardening notes, and sanitized VPS evidence. The main gaps are expected next-step work: choose the next slice, create its detailed plan, complete public-launch evidence, and decide whether the June 16 hardening plus self-hosted n8n handoff needs a dedicated review artifact before broader launch activity.
