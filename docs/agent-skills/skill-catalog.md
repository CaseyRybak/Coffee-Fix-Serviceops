...
- `serviceops-domain-modeling`: use for CoffeeFix Pro domain modeling.
- `fastapi-hexagonal-usecase`: use for FastAPI use cases inside DDD/hexagonal boundaries.
- `figma-reference-implementation`: use for implementing the public UI from `reference/figma`.
- `rag-knowledge-ingestion`: use for RAG document ingestion and retrieval.
...
When write access is available, each folder can be copied into `.agents/skills/<skill-name>/SKILL.md`.
